...
function createRoom(roomId, mode, label, questionBank, fixedRoundTime) {
  const state = {
    ...
    currentRound: 0,
    currentWord: null,     // typing: 정답 단어 / quiz: 정답(표시/공개용 대표 정답 1개)
    currentAnswerList: [], // quiz: 정답으로 인정할 모든 값의 배열 (answer가 배열이면 여러 개 가능)
  ...
  function startQuestionPhase() {
    ...
    if (state.mode === 'quiz') {
      ...
      // answer가 배열이면("사과","능금") 여러 정답을 모두 인정, 문자열이면 기존처럼 하나만 인정
      state.currentAnswerList = Array.isArray(q.answer) ? q.answer.filter(Boolean) : [q.answer];
      state.currentWord = state.currentAnswerList[0];
      ...
      state.currentAnswerList = [state.currentWord];
  ...
  function applyAnswer(p, correct, extra) {
    ...
    emitRoom('answerResult', Object.assign({
      ...
      correctAnswer: state.mode === 'wordchain' ? null : state.currentAnswerList.join(' / '),
    }, extra || {}));
  }
  ...
  function submitAnswer(socket, text) {
    ...
    const correct = state.currentAnswerList.some(a => (a || '').toString().trim().toLowerCase() === norm);
